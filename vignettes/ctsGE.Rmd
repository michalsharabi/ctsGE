...
data is loaded from the ‘extdata’ directory of the **ctsGE** package:
...
The `desc` argument in the function **readTSGE** is used in order to add genes 
...
standardization method is a median-based scaling; alternatively, a mean-based 
scaling can be used. The new standardized values represent the distance of each gene 
...
(see Section [4.1](#checking-the-optimal-cutoff)). 
...
(with the `cutoff` parameter) will change the index for each gene-expression 
...
groups. The range of the cutoff values is given by `min_cutoff` and `max_cutoff` 
...
### Index properties 
To explore how informative the expression profiles are, we counted the number
of zeros in each index. In this tutorial example, the index can have no zeros, 
...
This example focus on `1100-1-1` index. This index indicates expression profile
