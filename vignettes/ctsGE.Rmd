...
testing different values from 0.5 to 0.7 in increments of 0.05, there is an 
option to test more or less cutoff values by setting the `cutoff_check` 
parameter to larger or smaller values respectively.
...
generate index groups of equal size. The function `PreparingTheIndexes` test
range of cutoff values from 0.5 to 0.7 in increments of 0.05 
(overall 5 different cutoffs), for each cutoff the function generates an
expression index and performs a chi-squared test, cutoff with the lowest 
chi-squared is chosen. To test more or less cutoffs the `cutoff_check` parameter
can be set to larger or smaller values respectively.

    In the next example, cutoff 0.55 gives the lowest chi-squared:

```{r,message=FALSE,warning=FALSE}

prts <- PreparingTheIndexes(x = rts, cutoff_check = 0.7, mad.scale = TRUE)
